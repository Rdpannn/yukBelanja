...
export default function Register() {
  ...
  // step 1 = form email + nama toko, step 2 = modal nunggu verifikasi, step 3 = set nama & password
  ...
  // udah login dari awal, biar seller login yang nentuin dia ke dashboard atau buka toko
  if (session && step === 1) return <Navigate to="/seller/login" replace />

  // google gak bisa bawa metadata role, jadi lewat seller login
  // abis login role nya masih buyer, terus diarahin ke /toko buat isi nama toko
  ...
  // metadata role + store dibaca trigger di database, jadi akunnya lahir langsung seller
  async function kirimLink(e) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: window.location.origin + '/seller/register?lanjut=1',
        data: { role: 'seller', store: toko, phone: hp },
      },
    })
    setLoading(false)
    if (error) {
  ...
  // simpan nama + password, terus masuk dashboard
  ...
  return (
    <div className="flex justify-center px-4 pt-16 md:pt-24">
      <div className="bg-panel rounded-(--radius-kartu) p-8 md:p-10 w-full max-w-[420px] text-center">
        <h1 className="font-bold text-2xl">Mulai jualan yuk!</h1>
        <p className="text-kabut mt-2">Sekali daftar, tokomu langsung buka.</p>
        ...
        <p className="text-kabut text-sm mt-2">
          Cuma mau belanja?{' '}
          <Link to="/register" className="text-neon font-medium">
            Daftar di sini
          </Link>
        </p>
      </div>
